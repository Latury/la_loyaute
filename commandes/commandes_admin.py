...
class CommandesAdmin(commands.Cog):
    ...
    async def config_cmd(self, interaction: discord.Interaction):
        ...
        # ── 🔹 Création de l'embed
        embed = creer_embed(
            titre="⚙️ Configuration du bot",
            description="Paramètres actuels de La Loyauté",
            couleur=config.COULEUR_PRINCIPALE
        )
        ...
        embed.add_field(
            name="📋 Général",
            value=f"**Nom :** {config.NOM_BOT}\n"
                  f"**Version :** {config.VERSION_BOT}\n"
                  f"**Développeur :** {config.DEVELOPPEUR}\n"
                  f"**Mode debug :** {'Activé' if config.DEBUG_MODE else 'Désactivé'}",
            inline=False
        )

        # ── 🔹 Configuration des prefix
        embed.add_field(
            name="🔤 Prefix",
            value=f"**Base :** `{config.PREFIX_BASE}`\n"
                  f"**Admin :** `{config.PREFIX_ADMIN}`",
            inline=True
        )

        # ── 🔹 Configuration des rôles
        role_admin = "Non configuré" if config.ROLE_ADMIN_ID == 0 else f"<@&{config.ROLE_ADMIN_ID}>"
        role_modo = "Non configuré" if config.ROLE_MODERATEUR_ID == 0 else f"<@&{config.ROLE_MODERATEUR_ID}>"

        embed.add_field(
            name="👑 Rôles",
            value=f"**Admin :** {role_admin}\n"
                  f"**Modérateur :** {role_modo}",
            inline=False
        )
        ...
        # ── 🔹 Log de la consultation
        self.bot.logger.info(f"⚙️ Configuration consultée par {interaction.user}")

    # ╔═════════════════════════════════════════════════════════════════════════
    # ║ 🔄 Fonction 04 – Commande /reload
